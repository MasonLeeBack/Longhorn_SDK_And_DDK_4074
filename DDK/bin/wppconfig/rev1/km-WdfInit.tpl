    ...
    *Size = 0;
    ...
    switch(minorFunction)
    {
        ...
        case IRP_MN_DISABLE_EVENTS:
        {
            ...
            // find the "wtcb" ControlBlock which matches the Wnode GUID.
            //
            wtcb  = (PWPP_TRACE_CONTROL_BLOCK) cb;
            index = 0;
            while(wtcb) { 
                if (RtlEqualLuid( (LUID*)wtcb->ControlGuid, (LUID*)&Wnode->Guid )) {
                    break;
                }
                index++;
                wtcb = wtcb->Next; 
            ...
            WppDebug(0,("WppTraceCallBack: event for GUID[%d] {%S}\n",
                        index, WppGuidToStr((LPWSTR)&guid, wtcb->ControlGuid)));
            ...
            if (BufferLength >= sizeof(WNODE_HEADER)) {
                ...
                if (minorFunction == IRP_MN_DISABLE_EVENTS) {
                    ...
                    if (WppIsAdvancedTracingSupported()) {
                        ...
                        wtcb->Flags[0] = ((PTRACE_ENABLE_CONTEXT) &lh)->EnableFlags;
                        wtcb->Level = (UCHAR) ((PTRACE_ENABLE_CONTEXT) &lh)->Level;
                    }
