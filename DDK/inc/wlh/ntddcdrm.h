...
#define IOCTL_CDROM_LOCK_DEVICE           CTL_CODE(IOCTL_CDROM_BASE, 0x0017, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS )
#define IOCTL_CDROM_UNLOCK_DEVICE         CTL_CODE(IOCTL_CDROM_BASE, 0x0018, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS )
#define IOCTL_CDROM_READ_DISC_INFO        CTL_CODE(IOCTL_CDROM_BASE, 0x0019, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_CDROM_READ_TRACK_INFO       CTL_CODE(IOCTL_CDROM_BASE, 0x001A, METHOD_BUFFERED, FILE_READ_ACCESS)
...
//
// IOCTL_CDROM_LOCK_DEVICE can be used to get exclusive
// access to the CDROM device.
// Set IgnoreVolume to true to lock the device even if the
// file system is mounted. Setting this could cause data corruption!
//

typedef struct _LOCK_DEVICE {
    BOOLEAN IgnoreVolume;
} LOCK_DEVICE, *PLOCK_DEVICE;

//
// Address/number type for IOCTL_CDROM_READ_TRACK_INFO
//

typedef enum _ADDRESS_TYPE {
    AddressTypeLogicalBlockAddress = 0,
    AddressTypeLogicalTrackNumber  = 1,
    AddressTypeSessionNumber       = 2
} ADDRESS_TYPE, *PADDRESS_TYPE;

typedef struct _TRACK_ADDRESS {
    ULONG   AddressType;
    ULONG   LogicalBlockAddress;
} TRACK_ADDRESS, *PTRACK_ADDRESS;
