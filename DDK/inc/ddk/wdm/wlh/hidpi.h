...
//
// A call back function to compare two items for equivalence.  The function
// should return TRUE if the items are equivalent.
//

typedef BOOLEAN (* PHIDP_IS_SAME_ITEM) (
                  IN PVOID Item1,  
                  IN PVOID Item2
                  );
...
NTSTATUS __stdcall
HidP_ListDifference (
   IN    PVOID PreviousList,
   IN    PVOID CurrentList,
   OUT   PVOID BreakList,
   OUT   PVOID MakeList,
   IN    ULONG  ItemSize,
   IN    ULONG  ListLength,
   IN    PVOID ZeroItem,
   IN    PHIDP_IS_SAME_ITEM IsSameItem
   );
/*++
Routine Description:
    This function will return the difference between a two lists of items.
    In other words, it will return a list of items that are in the current list 
    but not the previous list as well as a list of items that are in the 
    previous list but not the current list.

Parameters:

    PreviousList   	The list of items before.
    CurrentList    	The list of items now.
    BreakList     	Previous - Current.
    MakeList       	Current - Previous.
    ItemSize		The size in bytes of an item in the list.
    ListLength     	epresents the length of the lists in array
                        ...
                        will be ignored.
    ZeroItem		An item that represents zero.
    IsSameItem		A pointer to a function that compares two items.
--*/
...
//
typedef BOOLEAN (* PHIDP_INSERT_SCANCODES_EX) (
                  ...
                  IN PUCHAR Usage, // The usage that generated the scan code
                  IN ULONG UsageSize // The size in bytes of this usage
                  );

		  
NTSTATUS __stdcall
HidP_TranslateUsageAndPagesToI8042ScanCodesEx (
    PVOID                         ChangedUsageList, // Those usages that changed
    ULONG                         UsageListLength,
    ULONG                         UsageSize,
    HIDP_KEYBOARD_DIRECTION       KeyAction,
    PHIDP_KEYBOARD_MODIFIER_STATE ModifierState,
    PHIDP_INSERT_SCANCODES_EX     InsertCodesProcedure,
