...
//
// New monitor control IOCTLs temporarily defined in Longhorn
//

// TODO: Now that this functionality is exposed via WMI from monitor function driver, these yet unreleased IOCTLs need to be removed

#define IOCTL_VIDEO_QUERY_SUPPORTED_BRIGHTNESS_EX \
    CTL_CODE(FILE_DEVICE_VIDEO, 0x128, METHOD_BUFFERED, FILE_ANY_ACCESS)

#define IOCTL_VIDEO_QUERY_DISPLAY_BRIGHTNESS_EX \
    CTL_CODE(FILE_DEVICE_VIDEO, 0x129, METHOD_BUFFERED, FILE_ANY_ACCESS)

#define IOCTL_VIDEO_SET_DISPLAY_BRIGHTNESS_EX \
    CTL_CODE(FILE_DEVICE_VIDEO, 0x12A, METHOD_BUFFERED, FILE_ANY_ACCESS)


//
// New monitor control IOCTLs defined in Longhorn
//

// For kernel-mode client usage only - not exposed via WMI
#define IOCTL_VIDEO_BRIGHTNESS_BIAS \
    CTL_CODE(FILE_DEVICE_VIDEO, 0x12b, METHOD_BUFFERED, FILE_ANY_ACCESS)

// TODO: Move this to vidpriv.h when it is self-contained and doesn't require inclusion of any other headers (e.g. viddef.h)
#define IOCTL_INTERNAL_VIDEO_GET_MONITOR_DESCRIPTOR \
    CTL_CODE(FILE_DEVICE_VIDEO, 0x12c, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
//
// New Longhorn driver model IOCTL that needs to be supported by the driver.
//

#define IOCTL_VIDEO_QUERY_SEGMENT_DESCRIPTOR                            \
    CTL_CODE(FILE_DEVICE_VIDEO, 0x301, METHOD_BUFFERED, FILE_ANY_ACCESS)
    
#define IOCTL_VIDEO_QUERY_PAGING_BUFFER_DESCRIPTOR                      \
...
// 
// defines for Flags fileld of _VIDEO_WIN32K_CALLBACKS
// 
#define VIDEO_SINGLE_BINARY_DRIVER        0x00000001
...
// TODO: Now that this functionality is exposed via WMI from monitor function driver, this newly introduced IOCTL needs to be removed
//
// Used for IOCTL_VIDEO_QUERY_SUPPORTED_BRIGHTNESS_EX
//

typedef struct {
    UCHAR HardwareAcDefault;    // 0-100%
    UCHAR HardwareDcDefault;    // 0-100%
    UCHAR Count;
    UCHAR Level[1];             // 0-100%
} VIDEO_BRIGHTNESS_SUPPORTED_LEVELS, *PVIDEO_BRIGHTNESS_SUPPORTED_LEVELS;

// TODO: Now that this functionality is exposed via WMI from monitor function driver, this newly introduced IOCTL needs to be removed
//
// Used for IOCTL_VIDEO_QUERY_DISPLAY_BRIGHTNESS_EX
//

typedef struct {
    UCHAR CurrentBrightness;
    BOOLEAN CompleteIrpOnChangeEvent;
} VIDEO_BRIGHTNESS_CURRENT, *PVIDEO_BRIGHTNESS_CURRENT;

// TODO: Now that this functionality is exposed via WMI from monitor function driver, this newly introduced IOCTL needs to be removed
//
// Used for IOCTL_VIDEO_SET_DISPLAY_BRIGHTNESS_EX
//

typedef struct {
    UCHAR Brightness;
    ULONGLONG Timeout;  // in seconds, zero means no timeout
} VIDEO_BRIGHTNESS_SET_BRIGHTNESS, *PVIDEO_BRIGHTNESS_SET_BRIGHTNESS;

//
// Used for IOCTL_VIDEO_BRIGHTNESS_BIAS
//

typedef struct {
    CHAR BrightnessBias;  // signed value representing brightness percentage.
} VIDEO_BRIGHTNESS_BIAS, *PVIDEO_BRIGHTNESS_BIAS;
...
// SegmentDescriptor 
//
// Longhorn memory manager segment description structure. Driver uses that 
// structure to expose their video memory resources to the video memory
// manager.
//

#define VIDEO_SEGMENT_APERTURE                         0x00000001
#define VIDEO_SEGMENT_LEGACYAGP                        0x00000002
#define VIDEO_SEGMENT_CPUVISIBLE                       0x00000004
#define VIDEO_SEGMENT_USEMAXIMUMSIZE                   0x00000008
#define VIDEO_SEGMENT_USE_BANKING                      0x00000010
#define VIDEO_SEGMENT_CACHE_COHERENT                   0x00000020
#define VIDEO_SEGMENT_READ_ONLY                        0x00000040
#define VIDEO_SEGMENT_VALID_FLAGS                      0x0000007F

typedef struct _VIDEO_SEGMENT_DESCRIPTOR
{
    LARGE_INTEGER        BaseAddress;           // base address of the segment.
    LARGE_INTEGER        BusRelativeAddress;    // bus relative address of the segment.
    SIZE_T               Size;                  // Size of the segment.
    ULONG                NbOfBanks;             // Number of bank.
    PLARGE_INTEGER       BankRangeTable;        // Range delimiting each bank.
    ULONG                Flags;                 // Segment flags.
    LARGE_INTEGER        MaxPhysicalAddress;    // Maximum physical address visible 
                                                // by the hardware (aperture segment 
                                                // only).
    PVOID                MapPages;
    PVOID                UnmapPages;
} VIDEO_SEGMENT_DESCRIPTOR, *PVIDEO_SEGMENT_DESCRIPTOR;

typedef struct _VIDEO_QUERY_SEGMENT_DESCRIPTOR
{
    ULONG NbSegment;
    PVIDEO_SEGMENT_DESCRIPTOR SegmentDescriptor;
} VIDEO_QUERY_SEGMENT_DESCRIPTOR, *PVIDEO_QUERY_SEGMENT_DESCRIPTOR;

// Paging buffer descriptor.
//
// Longhorn memory manager paging buffer parameter. Driver uses that structure
// to give information to the video memory manager about how to allocate
// the paging buffer.
//

typedef struct _VIDEO_QUERY_PAGING_BUFFER_DESCRIPTOR
{
    ULONG   Size;                               // Specify the size of the paging buffer.
    ULONG   SegmentId;                          // Specify the id of the segment where paging 
                                                // buffer should be allocated from.
} VIDEO_QUERY_PAGING_BUFFER_DESCRIPTOR, *PVIDEO_QUERY_PAGING_BUFFER_DESCRIPTOR;
