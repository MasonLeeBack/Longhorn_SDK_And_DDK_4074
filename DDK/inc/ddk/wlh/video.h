...
//
// Enumeration for miniport's native driver model version
//

typedef enum _DM_VERSION {
    DM_VERSION_XDDM = 1,
    DM_VERSION_LDDM
} DM_VERSION;
...
typedef struct _VIDEO_PORT_CONFIG_INFO {
    ...
    //
    // Interrupt Message Table.  Used by the miniport to identify the
    // cause of an interrupt.  Used only when Message-Signaled Interrupts
    // are enabled.
    //
    ...
    //
    // Video memory manager segment information.
    //

    ULONG NbVideoMemorySegment;
} VIDEO_PORT_CONFIG_INFO, *PVIDEO_PORT_CONFIG_INFO;

#define SIZE_OF_NT4_VIDEO_PORT_CONFIG_INFO FIELD_OFFSET(VIDEO_PORT_CONFIG_INFO, Master)
#define SIZE_OF_WXP_VIDEO_PORT_CONFIG_INFO FIELD_OFFSET(VIDEO_PORT_CONFIG_INFO, MessageTable)
...
typedef struct _VIDEO_HW_INITIALIZATION_DATA {
    ...
    PVIDEO_HW_DRV_ENABLE_DRIVER DrvEnableDriver;
    ...
    //
    // Drivers that support Message-Signaled Interrupts should
    // fill in these two fields.  Setting SynchronousMessageModel
    // to TRUE indicates that Videoport will guarantee that no
    // two messages will be delivered simultaneously.  Setting
    // it to FALSE indicates that videoport may allow multiple
    // messages to be simultaneously delivered to multiple
    // processors, or possibly to allow one message to pre-empt
    // another.
    //
    // If a miniport does not fill in MessageServiceRoutine,
    // videoport will not request Message-Signaled Interrupts
    // from the PnP Manager.
    //

    BOOLEAN SynchronousMessageModel;
    PVIDEO_HW_MESSAGE_INTERRUPT MessageServiceRoutine;

} VIDEO_HW_INITIALIZATION_DATA, *PVIDEO_HW_INITIALIZATION_DATA;

#define SIZE_OF_NT4_VIDEO_HW_INITIALIZATION_DATA     FIELD_OFFSET(VIDEO_HW_INITIALIZATION_DATA, HwStartDma)
#define SIZE_OF_W2K_VIDEO_HW_INITIALIZATION_DATA     FIELD_OFFSET(VIDEO_HW_INITIALIZATION_DATA, DrvEnableDriver)
#define SIZE_OF_WXP_VIDEO_HW_INITIALIZATION_DATA     FIELD_OFFSET(VIDEO_HW_INITIALIZATION_DATA, SynchronousMessageModel)
...
typedef enum {
    VideoCycleBrightness,
    VideoDecreaseBrightness,
    VideoIncreaseBrightness
} VIDEO_PORT_BRIGHTNESS_ACTION, *PVIDEO_PORT_BRIGHTNESS_ACTION;

typedef
VP_STATUS
(*PBRIGHTNESS_ACTION)(
    IN  PVOID  ActionContext,
    IN  VIDEO_PORT_BRIGHTNESS_ACTION Action
    );

typedef
VP_STATUS
(*PREGISTER_BRIGHTNESS_CALLBACK)(
    IN  PVOID  Context,
    IN  PBRIGHTNESS_ACTION ActionCallback,
    IN  PVOID  ActionContext
    );

typedef
VP_STATUS
(*PUNREGISTER_BRIGHTNESS_CALLBACK)(
    IN  PVOID  Context
    );

#define VIDEO_PORT_BRIGHTNESS_INTERFACE_VERSION_1 1
DEFINE_GUID( GUID_VIDEO_BRIGHTNESS_INTERFACE, 0xc908f109, 0xe086, 0x42b8, 0x97, 0xf0, 0x6b, 0x61, 0x98, 0x01, 0x51, 0xd6 );
...
typedef struct
{
    ...
    OUT PINTERFACE_REFERENCE       InterfaceReference;
    OUT PINTERFACE_DEREFERENCE     InterfaceDereference;
    OUT PGET_POSSIBLE_BRIGHTNESS   GetPossibleBrightness;
    OUT PSET_BRIGHTNESS            SetBrightness;
    OUT PGET_BRIGHTNESS            GetBrightness;
    OUT PREGISTER_BRIGHTNESS_CALLBACK    RegisterBrightnessCallback;
    OUT PUNREGISTER_BRIGHTNESS_CALLBACK  UnregisterBrightnessCallback;
} VIDEO_PORT_BRIGHTNESS_INTERFACE, *PVIDEO_PORT_BRIGHTNESS_INTERFACE;
...
BOOLEAN
VIDEOPORT_API
VideoPortSynchronizeMessageExecution(
    PVOID HwDeviceExtension,
    PMINIPORT_SYNCHRONIZE_ROUTINE SynchronizeRoutine,
    PVOID Context,
    ULONG MessageNumber
