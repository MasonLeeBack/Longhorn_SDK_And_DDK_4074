...
#pragma alloc_text(PAGE, CdRomDeviceControlLockDevice)
#pragma alloc_text(PAGE, CdRomDeviceControlUnLockDevice)
...
--*/
{
    ...
    //
    if (cdData->ExclusiveOwner != NULL && cdData->ExclusiveOwner != irpStack->FileObject) {
        ...
        if (CdRomIsStorageIoctl(Irp) == TRUE) {
...
RetryControl:
    ...
    case IOCTL_CDROM_LOCK_DEVICE: {

        TraceLog((CdromDebugTrace,
                    "IOCTL_CDROM_LOCK_DEVICE to via irp %p\n", Irp));

        status = CdRomDeviceControlLockDevice(DeviceObject, Irp);
        break;
    }

    case IOCTL_CDROM_UNLOCK_DEVICE: {

        TraceLog((CdromDebugTrace,
                    "IOCTL_CDROM_UNLOCK_DEVICE to via irp %p\n", Irp));

        status = CdRomDeviceControlUnLockDevice(DeviceObject, Irp);
        break;
    }

    case IOCTL_CDROM_READ_DISC_INFO: {

        TraceLog((CdromDebugTrace,
                    "IOCTL_CDROM_READ_DISC_INFO to via irp %p\n", Irp));
        ...
        if (irpStack->Parameters.DeviceIoControl.OutputBufferLength <
            RTL_SIZEOF_THROUGH_FIELD(DISC_INFORMATION, Length)) {
            status = STATUS_BUFFER_TOO_SMALL;
            Irp->IoStatus.Information = RTL_SIZEOF_THROUGH_FIELD(DISC_INFORMATION, Length);
            break;
        }
        ...
        if (irpStack->Parameters.DeviceIoControl.OutputBufferLength &
            fdoExtension->AdapterDescriptor->AlignmentMask) {
            //
            // buffer is not aligned as required by the controller
            //
            status = STATUS_INVALID_PARAMETER;
            break;
        }
    ...
    case IOCTL_CDROM_READ_TRACK_INFO: {

        PTRACK_ADDRESS inputBuffer = Irp->AssociatedIrp.SystemBuffer;

        TraceLog((CdromDebugTrace,
                    "IOCTL_CDROM_READ_TRACK_INFO to via irp %p\n", Irp));
        ...
        if (irpStack->Parameters.DeviceIoControl.InputBufferLength <
            sizeof(TRACK_ADDRESS)) {
            status = STATUS_INFO_LENGTH_MISMATCH;
            break;
        }
        if (irpStack->Parameters.DeviceIoControl.OutputBufferLength <
            RTL_SIZEOF_THROUGH_FIELD(TRACK_INFORMATION2, Length)) {
            status = STATUS_BUFFER_TOO_SMALL;
            Irp->IoStatus.Information = RTL_SIZEOF_THROUGH_FIELD(TRACK_INFORMATION2, Length);
            break;
        }
        if (irpStack->Parameters.DeviceIoControl.OutputBufferLength &
            fdoExtension->AdapterDescriptor->AlignmentMask) {
    ...
    )
{
    ...
    if (SRB_STATUS(srb->SrbStatus) != SRB_STATUS_SUCCESS) {
        ...
        if (realIrpStack->MajorFunction == IRP_MJ_DEVICE_CONTROL) {
            if ((realIrpStack->Parameters.DeviceIoControl.IoControlCode == IOCTL_CDROM_GET_LAST_SESSION) ||
                ...
                (realIrpStack->Parameters.DeviceIoControl.IoControlCode == IOCTL_CDROM_READ_DISC_INFO)   ||
                (realIrpStack->Parameters.DeviceIoControl.IoControlCode == IOCTL_CDROM_READ_TRACK_INFO)  ||
    ...
    if (NT_SUCCESS(status)) {
        ...
        case IOCTL_CDROM_GET_CONFIGURATION: {
            ...
            //
            // The real data transfer length is actually the minimum
            // of the Srb->DataTransferLength and the header's length
            // field + RTL_SIZEOF_THROUGH_FIELD length field.
            //
            PGET_CONFIGURATION_HEADER header = (PGET_CONFIGURATION_HEADER)srb->DataBuffer;
            ULONG realTransferLength =
                (header->DataLength[0] << (8*3)) |
                (header->DataLength[1] << (8*2)) |
                (header->DataLength[2] << (8*1)) |
                (header->DataLength[3] << (8*0)) ;
            if (((ULONG)-1) - RTL_SIZEOF_THROUGH_FIELD(GET_CONFIGURATION_HEADER, DataLength) >= realTransferLength)
            {
                realTransferLength = (ULONG)-1;
            }
            else
            {
                realTransferLength += RTL_SIZEOF_THROUGH_FIELD(GET_CONFIGURATION_HEADER, DataLength);
            }

            //
            // Note: If realTransferLength is larger than srb->DataTransferLength at this
            // point, we *could* fail the request with STATUS_BUFFER_OVERFLOW, but then
            // user-mode programs wouldn't get the partial data, and then unable to
            // determine how much data they *should* allocate to succeed.
            // In addition, this would cause failure cases where previously this IOCTL
            // was returning a STATUS_SUCCESS.  This would cause support nightmares,
            // and make it very difficult to write simple code in user mode.
            // Therefore, we return success in these cases.
            //
            realTransferLength = min(realTransferLength, srb->DataTransferLength);

            //
            // now copy the memory back to the caller
            //
            RtlMoveMemory(realIrp->AssociatedIrp.SystemBuffer,
                          srb->DataBuffer,
                          realTransferLength);
            realIrp->IoStatus.Information = realTransferLength;
            break;
        }
        ...
        case IOCTL_CDROM_GET_VOLUME: {
            ...
            bytesTransferred =
                (ULONG)((PCHAR) audioOutput - (PCHAR) volumeControl) +
                sizeof(AUDIO_OUTPUT);
        ...
        case IOCTL_CDROM_READ_DISC_INFO: {
            ...
            // field + RTL_SIZEOF_THROUGH_FIELD length field.
            //
            PDISC_INFORMATION discInfo = (PDISC_INFORMATION)srb->DataBuffer;

            ULONG realTransferLength = ((discInfo->Length[0] << 8) | discInfo->Length[1]);
            realTransferLength += RTL_SIZEOF_THROUGH_FIELD(DISC_INFORMATION, Length);
            realTransferLength = min(realTransferLength, srb->DataTransferLength);
        ...
        case IOCTL_CDROM_READ_TRACK_INFO: {
            ...
            // field + RTL_SIZEOF_THROUGH_FIELD length field.
            //
            PTRACK_INFORMATION2 trackInfo = (PTRACK_INFORMATION2)srb->DataBuffer;

            ULONG realTransferLength = ((trackInfo->Length[0] << 8) | trackInfo->Length[1]);
            realTransferLength += RTL_SIZEOF_THROUGH_FIELD(TRACK_INFORMATION2, Length);
            realTransferLength = min(realTransferLength, srb->DataTransferLength);
    ...
    )
{
    ...
    PLOCK_DEVICE lockDevice;
    KEVENT event;

    if (KeGetCurrentIrql() != PASSIVE_LEVEL) {
        TraceLog((CdromDebugError, "CdRomDeviceControlLockDevice: IOCTL must be called at passive level.\n"));
        status = STATUS_INVALID_DEVICE_REQUEST;
        goto getout;
    }

    if (irpStack->Parameters.DeviceIoControl.InputBufferLength < sizeof(LOCK_DEVICE)) {

        TraceLog((CdromDebugWarning, "CdRomDeviceControlLockDevice: Input buffer too small\n"));
        status = STATUS_INVALID_PARAMETER;
        goto getout;
    }
    ...
    irpStack->Parameters.Others.Argument4 = &event;

    // Send the request to StartIo routine
    ...
    if (fsContext == NULL) {
        ...
        TraceLog((CdromDebugError,
                   "CdRomDeviceControlLockDevice: Unable to lock device, invalid file object\n"));
    ...
    lockDevice = (PLOCK_DEVICE) Irp->AssociatedIrp.SystemBuffer;

    if ( (lockDevice->IgnoreVolume == FALSE) &&
         (ClassGetVpb(fdoExtension->DeviceObject) != NULL) &&
         (ClassGetVpb(fdoExtension->DeviceObject)->Flags & VPB_MOUNTED) ) {
            TraceLog((CdromDebugError,
                       "CdRomDeviceControlLockDevice: Unable to lock device, file system mounted\n"));
    ...
    if (InterlockedCompareExchangePointer(&cdData->ExclusiveOwner, fileObject, NULL) == NULL) {
        TraceLog((CdromDebugTrace,
                   "CdRomDeviceControlLockDevice: Device locked, entering exclusive mode!\n"));
        ...
        TraceLog((CdromDebugError,
                   "CdRomDeviceControlLockDevice: Unable to lock device, device alredy locked.\n"));
...
NTSTATUS
CdRomDeviceControlUnLockDevice(
    IN PDEVICE_OBJECT Fdo,
    IN PIRP Irp
    )
{
    ...
    if (KeGetCurrentIrql() != PASSIVE_LEVEL) {
        TraceLog((CdromDebugError, "CdRomDeviceControlUnLockDevice: IOCTL must be called at passive level.\n"));
    ...
    if (cdData->ExclusiveOwner != NULL) {
        ...
        if (irpStack->FileObject == cdData->ExclusiveOwner) {

            status = CdRomUnLockDevice(Fdo, Irp);

        } else {

            //
            // This request is not from the exclusive owner.
            // We can't unlock the device.
            //

            TraceLog((CdromDebugError,
                       "CdRomDeviceControlUnLockDevice: Unable to unlock device, invalid file object\n"));
    ...
    )
{
    ...
    if (irpStack->MajorFunction == IRP_MJ_DEVICE_CONTROL) {
        ...
        if (fsContext == NULL) {
            ...
            TraceLog((CdromDebugError, "CdRomUnLockDevice: Unable to unlock device, invalid file object\n"));
    ...
    TraceLog((CdromDebugTrace, "CdRomUnLockDevice: Device unlocked\n"));
...
Routine Description:
    ...
    This routine check with the given ioctl is a storage ioctl.
    If the device type of the IOCTL is one of the stoarge type
...
--*/
{
    ...
    // Note: These commnads should not generate any requests to the device.
    //
