...
#if DBG
    {
        ...
        if (dbgBuffer != NULL) {
            ...
            if (NT_SUCCESS(dbgStatus)) {
                CdRompPrintAllFeaturePages(dbgBuffer, size);
            }
    ...
    )
{
    ...
    //
    {
        ...
        if ((defectHeader != NULL)  && (writableHeader != NULL) &&
            ...
            mmcData->ValidationSchema = FeatureDefectManagement;
            ...
            header = CdRomFindFeaturePage(mmcData->CapabilitiesBuffer,
                                          srb->DataTransferLength,
            ...
            if (header != NULL && header->Current &&
                ...
                mmcData->Blocking = (feature->Blocking[0] << 8) | feature->Blocking[1];
            ...
            //
            // Step 1: Look for the RandomWritable feature
            //
            header = CdRomFindFeaturePage(mmcData->CapabilitiesBuffer,
                                          srb->DataTransferLength,
                                          FeatureRandomWritable);
            ...
            if (header != NULL && header->Current) {
                ...
                mmcData->WriteAllowed     = TRUE;
                mmcData->ValidationSchema = FeatureRandomWritable;

                goto FinishDriveUpdate;
            }

            //
            // Step 2: Look for the RigidRestrictedOverwrite feature 
            //
            header = CdRomFindFeaturePage(mmcData->CapabilitiesBuffer,
                                          srb->DataTransferLength,
            ...
            if (header != NULL && header->Current) {
                ...
                mmcData->WriteAllowed     = TRUE;
                mmcData->ValidationSchema = FeatureRigidRestrictedOverwrite;

                goto FinishDriveUpdate;
            }

            //
            // Step 3: Look for the RestrictedOverwrite feature 
            //
            header = CdRomFindFeaturePage(mmcData->CapabilitiesBuffer,
                                          srb->DataTransferLength,
            ...
            if (header != NULL && header->Current) {
                ...
                mmcData->WriteAllowed     = TRUE;
                mmcData->ValidationSchema = FeatureRestrictedOverwrite;

                goto FinishDriveUpdate;
            }

            //
            // Step 4: Look for the IncrementalStreamingWritable feature 
            //
            header = CdRomFindFeaturePage(mmcData->CapabilitiesBuffer,
                                          srb->DataTransferLength,
            ...
            if (header != NULL && header->Current) {
                ...
                mmcData->WriteAllowed     = TRUE;
                mmcData->ValidationSchema = FeatureIncrementalStreamingWritable;

                goto FinishDriveUpdate;
            }

            //
            // Step 5: Look for the DvdPlusR feature and ensure that writes are allowed. This is
            //         needed because DVD+R do not have the IncrementalStreamingWritable feature
            //         explicitly set in the capabilities list
            //
            additionalLength = FIELD_OFFSET(FEATURE_DATA_DVD_PLUS_R, Reserved2[0]) - sizeof(FEATURE_HEADER);
            ...
            if (header != NULL && header->Current &&
                header->AdditionalLength >= additionalLength && ((PFEATURE_DATA_DVD_PLUS_R)header)->Write) {
