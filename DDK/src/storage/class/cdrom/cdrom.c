...

{
    ...
    if (!isMmcDevice) {
        ...
        TraceLog((CdromDebugDeviceInfo,
                  "FDO %p GET_CONFIGURATION Data:  %!HEXDUMP!\n",
                  Fdo,
                  log_xstr(cddata->Mmc.CapabilitiesBufferSize, cddata->Mmc.CapabilitiesBuffer)
                  ));
    ...
    )
{
    ...
    if (cdData->ExclusiveOwner != NULL && cdData->ExclusiveOwner != currentIrpStack->FileObject) {
        ...
        if ( (currentIrpStack->MajorFunction == IRP_MJ_DEVICE_CONTROL) &&
             (CdRomIsStorageIoctl(Irp) == TRUE) ) {
            KdPrintEx((DPFLTR_CDROM_ID, CdromDebugError, "CdRomStartIo: Access Denied! Device in exclusive mode.\n"));
    ...
    if (currentIrpStack->MajorFunction == IRP_MJ_READ ||
...
#if 0
        schema = cdData->Mmc.ValidationSchema;
        ...
        if (currentIrpStack->MajorFunction == IRP_MJ_WRITE && !(cdData->Mmc.WriteAllowed)) {
            ...
            TraceLog((CdromDebugError, "CdRomStartIo: Write request to read-only media\n"));
            isValid = FALSE;
        }
            ...
            switch (schema) {
                ...
                case FeatureRandomWritable:
                    ...
                    if (startingOffset.QuadPart >= commonExtension->PartitionLength.QuadPart ||
                        ...
                        TraceLog((CdromDebugError, "CdRomStartIo: Request is out of bounds\n"));
                        isValid = FALSE;
                ...
                case FeatureRigidRestrictedOverwrite:
                    ...
                    if (((transferByteCount >> fdoExtension->SectorShift) % cdData->Mmc.Blocking) != 0) {
                        ...
                        TraceLog((CdromDebugError, "CdRomStartIo: Number of blocks is not a multiple of the blocking size (%x)\n", cdData->Mmc.Blocking));
                        isValid = FALSE;
                    }
                ...
                case FeatureRestrictedOverwrite:
                    ...
                    if ((Int64ShrlMod32(startingOffset.QuadPart, fdoExtension->SectorShift) % cdData->Mmc.Blocking) != 0) {
                        ...
                        TraceLog((CdromDebugError, "CdRomStartIo: Starting block is not a multiple of the blocking size (%x)\n", cdData->Mmc.Blocking));
                        isValid = FALSE;
                    }
                    break;
                ...
                default:
                    ...
                    //
                    TraceLog((CdromDebugError, "CdRomStartIo: Unknown validation schema (%x)\n", schema));
                    isValid = FALSE;
                    break;
            }
        }
        ...
        case IOCTL_CDROM_UNLOCK_DEVICE: {
            ...
            PKEVENT event = currentIrpStack->Parameters.Others.Argument4;
        ...
        case IOCTL_CDROM_READ_DISC_INFO: {
            ...
            cdb->CDB10.OperationCode = SCSIOP_READ_DISC_INFORMATION;
            cdb->READ_DISC_INFORMATION.AllocationLength[0] = (UCHAR) (transferByteCount >> 8);
            cdb->READ_DISC_INFORMATION.AllocationLength[1] = (UCHAR) (transferByteCount &  0xFF);
        ...
        case IOCTL_CDROM_READ_TRACK_INFO: {
            ...
            cdb->CDB10.OperationCode = SCSIOP_READ_TRACK_INFORMATION;
            cdb->READ_TRACK_INFORMATION.Track = (UCHAR) inputBuffer->AddressType;
            cdb->READ_TRACK_INFORMATION.BlockAddress[0] = ((PFOUR_BYTE)&inputBuffer->LogicalBlockAddress)->Byte3;
            cdb->READ_TRACK_INFORMATION.BlockAddress[1] = ((PFOUR_BYTE)&inputBuffer->LogicalBlockAddress)->Byte2;
            cdb->READ_TRACK_INFORMATION.BlockAddress[2] = ((PFOUR_BYTE)&inputBuffer->LogicalBlockAddress)->Byte1;
            cdb->READ_TRACK_INFORMATION.BlockAddress[3] = ((PFOUR_BYTE)&inputBuffer->LogicalBlockAddress)->Byte0;
            cdb->READ_TRACK_INFORMATION.AllocationLength[0] = (UCHAR) (transferByteCount >> 8);
            cdb->READ_TRACK_INFORMATION.AllocationLength[1] = (UCHAR) (transferByteCount &  0xFF);
...

{
    ...
    if (cdData->ExclusiveOwner != NULL && cdData->ExclusiveOwner != irpStack->FileObject) {
        ...
        TraceLog((CdromDebugWarning, "CdRomReadWriteVerification: Access Denied! Device in exclusive mode.\n"));
...
--*/
{
    ...
    for (count = 0; count < 6; count++) {
            ...
            (status == STATUS_BUFFER_OVERFLOW))
        {
            ...
            TraceLog((CdromDebugDeviceInfo,
                      "FDO %p CDVD Capabilities Page Data:  %!HEXDUMP!\n",
                      Fdo,
                      log_xstr(bufferLength,buffer)
                      ));
