...
Retry:
    ...
    case IOCTL_DISK_GROW_PARTITION: {
        ...
        //
        // Ensure that if the number of bytes to grow is negative,  then it isn't
        // in excess of the the current partition length. This check is necessary
        // because this ioctl may also be used to shrink a partition
        //

        if (inputBuffer->BytesToGrow.QuadPart < 0 &&
            pdoCommonExtension->PartitionLength.QuadPart + inputBuffer->BytesToGrow.QuadPart < 0) {
        ...
        //
        // Ensure that the new stopping offset does not fall behind the starting
        // offset... unless we want to allow zero length partitions
        //

        if (newStoppingOffset.QuadPart < pdoCommonExtension->StartingOffset.QuadPart) {
