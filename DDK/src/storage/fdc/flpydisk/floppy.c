...

{
    ...
    if ( NT_SUCCESS(ntStatus) ) {
        ...
        if ( NT_SUCCESS(ntStatus) ) {
            ...
            if ( disketteExtension->TargetObject == NULL ) {
...

{
    ...
    HANDLE      threadHandle;
    NTSTATUS    status;
    BOOLEAN     launchNewThread;
    KIRQL       oldIrql;

    //
    // Verify if the system is powering down. If so we fail the irps.
    //
    ...
    //
    // The first request that takes ThreadReferenceCount from -1 to 0
    // should launch the floppy thread. Otherwise just increment 
    // ThreadReferenceCount for that request.
    //
    if (++(DisketteExtension->ThreadReferenceCount) == 0) {
        ...
        //
        // Increment ThreadReferenceCount once again for this request.
        //
    ...
    ExInterlockedInsertTailList(&DisketteExtension->ListEntry,
                                &Irp->Tail.Overlay.ListEntry,
                                &DisketteExtension->ListSpinLock);

    KeReleaseSemaphore(&DisketteExtension->RequestSemaphore,
                       (KPRIORITY) 0, 
                       1,
                       FALSE );
    ...
    if (NT_SUCCESS(status)) {
        ...
        //
        // Failed to launch the thread. Need to complete all
        // the queued requests with failure status.
        //
        ...
        //
        // Decrement ThreadReferenceCount. This is for the increment
        // that took ThreadReferenceCount from -1 to 0.
        //
        ASSERT(DisketteExtension->ThreadReferenceCount > 0);
        DisketteExtension->ThreadReferenceCount--;

        //
        // Now for each request, which resulted in ThreadReferenceCount
        // being incremented once, decrement ThreadReferenceCount until
        // it goes to -1
        //
        while (DisketteExtension->ThreadReferenceCount >= 0) {

            ExReleaseFastMutex(&DisketteExtension->ThreadReferenceMutex);

            //
            // Wait until the request has been added to the queue and
            ...
            if (waitStatus != STATUS_TIMEOUT) {
                ...
                failedIrp = CONTAINING_RECORD(request,
                                              IRP,
                                              Tail.Overlay.ListEntry);

                failedIrp->IoStatus.Status = status;

                IoCompleteRequest(failedIrp, IO_DISK_INCREMENT);

                ExAcquireFastMutex(&DisketteExtension->ThreadReferenceMutex);

                //
                // This decrement corresponds to the increment done
                // for this request.
                //
                ASSERT(DisketteExtension->ThreadReferenceCount >= 0);
                DisketteExtension->ThreadReferenceCount--;
