...
#define MUX_MAJOR_DRIVER_VERSION       2
...
VOID
PtSendNBLComplete(
    IN NDIS_HANDLE ProtocolBindingHandle,
    IN PNET_BUFFER_LIST NetBufferLists,
    IN BOOLEAN DispatchLevel
    );
...
typedef struct _MUX_NDIS_REQUEST
{
    ...
    PNDIS_REQUEST_EX            OrigRequest; //Request that originated this request
    NDIS_REQUEST_EX             Request;
...
typedef struct _VELAN
{
    ...
    PNET_BUFFER_LIST            PendingNBLs;
    PNET_BUFFER_LIST            PendingNBLsTail;
...
#if IEEE_VLAN_SUPPORT
    BOOLEAN                     MultipleVlanIds;
    ULONG                       VlanIdList[4096];
    ULONG                       VlanIdListCount;
    UCHAR                       VlanIds[512];
    NDIS_SPIN_LOCK              VlanIdListLock;
#endif
...
#define MP_GET_NET_BUFFER_LIST_REF_COUNT(_NetBufferList) \
    ((ULONG)(ULONG_PTR)((_NetBufferList)->MiniportReserved[0]))


#ifdef IEEE_VLAN_SUPPORT
NDIS_STATUS MPHandleSendTaggingNB(IN PVELAN pVElan,
...
// MDL management functions which are used in advancing and
// retreating a net buffer list

NDIS_STATUS MuxInitializeMdlBuffer();

PMDL MuxAllocateMdl(IN OUT PULONG BufferSize);

VOID MuxFreeMdl(IN PMDL Mdl);

VOID MuxDeInitializeMdlBuffer();

#if IEEE_VLAN_SUPPORT
BOOLEAN MuxRecognizedVlanId(PVELAN pVElan, ULONG VlanId);
#endif
