...
#if NDISMON
#define FILTER_FRIENDLY_NAME        L"NDIS Sample Minitor LightWeight Filter"
...
#if NDISMON1
#define FILTER_FRIENDLY_NAME        L"NDIS Sample Minitor 1 LightWeight Filter"
...
typedef struct _MS_FILTER
{
    ...
    NDIS_HANDLE                     RcvNetBufferListPoolHandle;
    NDIS_STATUS                     Status;
    NDIS_EVENT                      Event;
    ULONG                           BackFillSize;
...
//
// Check if it is OK to pause the filter
//
#define FILTER_READY_TO_PAUSE(_Filter)      \
    (((_Filter)->State == FilterPausing) \
        && ((_Filter->OutstandingSends == 0)         \
            && (_Filter->OutstandingRcvs == 0)))
