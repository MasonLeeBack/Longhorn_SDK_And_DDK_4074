...
//
// These strings don't need to be localized, since they correspond
// to definitions in header files, not to English.
//

#define IRQ_POLICY_COUNT  5
PWSTR IrqPolicyValues[IRQ_POLICY_COUNT] = {
    TEXT("IrqPolicyMachineDefault"),
    TEXT("IrqPolicyAllCloseProcessors"),
    TEXT("IrqPolicyOneCloseProcessor"),
    TEXT("IrqPolicyAllProcessorsInMachine"),
    TEXT("IrqPolicySpecifiedProcessors")
};

#define IRQ_PRIORITY_COUNT  4
PWSTR IrqPriorities[IRQ_PRIORITY_COUNT] = {
    TEXT("IrqArbPriorityUndefined"),
    TEXT("IrqArbPriorityLow"),
    TEXT("IrqArbPriorityNormal"),
    TEXT("IrqArbPriorityHigh")
};
...
BOOL DumpDeviceLinkNode(DEVINST DevInst,HMACHINE MachineHandle)
{
    GetLinkNodeIDProto LinkNodeFn;
    HMODULE cmAPI = NULL;
    CONFIGRET   status;
    WCHAR   buffer[150] = {0};
    BOOL    retVal;
    ...
    LinkNodeFn = (GetLinkNodeIDProto)GetProcAddress(cmAPI,CMGETLINKNODEID);
    if(!LinkNodeFn)
    {
        retVal = FALSE;
        goto DumpDeviceLinkNodeExit;
    }

    status = LinkNodeFn(DevInst, (LPWSTR)buffer, 150*sizeof(WCHAR), 0, MachineHandle);

    if (status == CR_SUCCESS) {

        if (buffer[0] != 0) {
            Padding(2);
            _tprintf(TEXT("LinkNode : %s\n"), buffer);
        }
    }

    retVal = TRUE;

DumpDeviceLinkNodeExit:
...
--*/
{
    ...
    DumpDeviceLinkNode(DevInfo->DevInst,devInfoListDetail.RemoteMachineHandle);
...
BOOL DumpDevicePolicy(HDEVINFO Devs,PSP_DEVINFO_DATA DevInfo)
/*++

Routine Description:

    Dump Policy to stdout
...
--*/
{
    ...
    //
    // make use of policy functions that are new, and thus don't
    // build or work under .Net Server or earlier systems
    //
    cmAPI = LoadLibrary(TEXT("setupapi.dll"));
    if(!cmAPI) {
        return FALSE;
    ...
    {
        retVal = FALSE;
        goto DumpDevicePolicyExit;
    }

    status = PolicyFn(DevInfo->DevInst,
                      FALSE,
                      &policy,
                      &priority,
                      &affinity,
    ...
    Padding(1);
    FormatToStream(stdout,MSG_DUMP_POLICY);

    if (policy < IRQ_POLICY_COUNT) {
        Padding(2);
        _tprintf(TEXT("IRQ Policy: %d - %s\n"), policy, IrqPolicyValues[policy]);
    }

    if (priority < IRQ_PRIORITY_COUNT) {
        Padding(2);
        _tprintf(TEXT("IRQ Priority: %d - %s\n"), priority, IrqPriorities[priority]);
    }

    Padding(2);
    _tprintf(TEXT("IRQ Affinity: %016I64x\n"), affinity);

    retVal = TRUE;

DumpDevicePolicyExit:
