...
#define FIND_POLICY         0x00000100 // display device's policy values
...
--*/
{
    ...
    if (pFindContext->control&FIND_POLICY) {
        DumpDevicePolicy(Devs,DevInfo);
    }
    pFindContext->count++;
    return EXIT_OK;
}
...
#define MODIFY_AFFINITY     0x00000001
#define MODIFY_PRIORITY     0x00000002

int PolicyCallback(HDEVINFO Devs,PSP_DEVINFO_DATA DevInfo,DWORD Index,LPVOID Context)
/*++

Routine Description:

    Callback for use by policy command
...
{
    HMODULE cmAPI = NULL;
    PolicyContext *pContext = (PolicyContext*)Context;
    SP_DEVINFO_LIST_DETAIL_DATA devInfoListDetail;
    DeviceAffinityPolicyProto PolicyFn;
    ULONG   policy, priority;
    ULONGLONG   affinity;
    ...
    //
    // make use of policy functions that are new, and thus don't
    // build or work under Windows Server 2003 or earlier systems
    //
    cmAPI = LoadLibrary(TEXT("setupapi.dll"));
    if(!cmAPI) {
        return EXIT_FAIL;
    }
    PolicyFn = (DeviceAffinityPolicyProto)GetProcAddress(cmAPI,DEVICEAFFINITYPOLICY);
    if(!PolicyFn)
    {
        goto PolicyCallbackExit;
    }

    //
    // Read full policy so that we can then write back
    // the changed parts.
    //

    retval = PolicyFn(DevInfo->DevInst,
                      FALSE,
                      &policy,
                      &priority,
                      &affinity,
    ...
    //
    // Now write back appropriate policy changes.
    //

    if (pContext->control & MODIFY_AFFINITY) {

        policy = pContext->policy;
        affinity = pContext->affinity;
    }

    if (pContext->control & MODIFY_PRIORITY) {

        priority = pContext->priority;
    }

    //
    // Write back the changed parts.
    //
...
int cmdPolicy(LPCTSTR BaseName,LPCTSTR Machine,int argc,TCHAR* argv[])
{
    GenericContext context;
    int failcode;
    ...
    if(lstrcmpi(argv[0],TEXT("get"))==0) {
    
        context.count = 0;
        context.control = FIND_DEVICE | FIND_POLICY;
        failcode = EnumerateDevices(BaseName,Machine,0,argc,argv,FindCallback,&context);
    ...
    } else if(lstrcmpi(argv[0],TEXT("set"))==0) {
        
        if (lstrcmpi(argv[1],TEXT("IRQPriority"))==0) {

            policy.control = MODIFY_PRIORITY;
            policy.count = 0;
            
            //
            // The next parameter should be a hex integer.
            //

            swscanf(argv[2],TEXT("%x"),&policy.priority);

            failcode = EnumerateDevices(BaseName,Machine,0,argc,argv,PolicyCallback,&policy);

            if (failcode == EXIT_OK) {
                FormatToStream(stdout,MSG_POLICY_SET,policy.count);
                FormatToStream(stdout,MSG_POLICY_RESTART);
            }

        } else if (lstrcmpi(argv[1],TEXT("IRQAffinity"))==0) {

            policy.control = MODIFY_AFFINITY;
            policy.count = 0;
            
            //
            // The next parameter should be a hex integer.
            // The one after that is an affinity.
            //

            swscanf(argv[2],TEXT("%x"),&policy.policy);
            swscanf(argv[3],TEXT("%016I64x"),&policy.affinity);
...
DispatchEntry DispatchTable[] = {
    ...
    { TEXT("policy"),       cmdPolicy,      MSG_POLICY_SHORT,      MSG_POLICY_LONG },
