...
VOID
SerialSetPendingIRPEvent( IN PKDPC Dpc, IN PSERIAL_DEVICE_EXTENSION PDevExt,
                          IN PVOID SystemContext1, IN PVOID SystemContext2)
/*++

Routine Description:

    SerialInsertQueueDpc has to set and event PendingIRPEvent in certain 
...
//      LOGENTRY(LOG_CNT, 'DpD1', PDpc, PDevExt->DpcCount, 0);
      ...
      if (pendingCnt == 0) {
          if ( KeGetCurrentIrql() <= DISPATCH_LEVEL ) {
              ...
              KeInsertQueueDpc( &PDevExt->SetPendingIRPEvent, NULL, NULL );
          }
