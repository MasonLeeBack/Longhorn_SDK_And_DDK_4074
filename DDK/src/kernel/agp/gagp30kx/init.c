...
--*/
{
    ...
    //
    // We'll let these early revision chips get away with claiming 3.5
    // capability versions, even though they are not compatible
    //
    } else if (!((TargetCap.Minor >= 0) && (TargetCap.Minor < 5))) {

        if ((Extension->SpecialTarget & AGP_FLAG_PRIVATE_IGNORE_CAPS_VERSION)
            != AGP_FLAG_PRIVATE_IGNORE_CAPS_VERSION) {
            AGPLOG(AGP_CRITICAL,
                   ("GAGP30KXTargetInit: Bad capability minor version (%d)!\n",
                    TargetCap.Minor));
            return STATUS_NOT_SUPPORTED;
        }
    }
    ...
    //
    // K8 MP support must be tested/verified
    //
    if ((KeNumberProcessors > 1) &&
        ((Extension->SpecialTarget & AGP_FLAG_PRIVATE_ENABLE_MP_SUPPORT) !=
         AGP_FLAG_PRIVATE_ENABLE_MP_SUPPORT)) {
        AGPLOG(AGP_CRITICAL,
               ("GAGP30KXTargetInit: Unsupported MP system\n"));
        return STATUS_NOT_SUPPORTED;
    }
