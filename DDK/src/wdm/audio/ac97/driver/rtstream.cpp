 ...
 */
NTSTATUS CreateRTStream
(
    OUT CRTStream  **RTStream
)
{
    PAGED_CODE ();

    DOUT (DBG_PRINT, ("[CreateRTStream]"));

    //
    // This is basically like the macro at stdunk with the change that we
    // don't cast to interface unknown but to interface CRTStream.
    //
    *RTStream = new (NonPagedPool, PoolTag) CRTStream (NULL);
    if (*RTStream)
    {
        (*RTStream)->AddRef ();
...
CRTStream::~CRTStream ()
{
    ...
    DOUT (DBG_PRINT, ("[CRTStream::~CRTStream]"));
    ...
    //
    // Delete the RT buffer since it's not needed anymore
    //
    if (audioBufferMdl)
    {
        PortStream->FreePagesFromMdl (audioBufferMdl);
    }
    if (BDListMdl && BDList)
    {
 ...
 */
NTSTATUS CRTStream::Init
(
    IN  CRTMiniport     *Miniport_,
    IN  PRTPORTSTREAM    PortStream_,
    IN  ULONG            Channel_,
    IN  BOOLEAN          Capture_,
...
/*****************************************************************************
 * CRTStream::NonDelegatingQueryInterface
 *****************************************************************************
 * Obtains an interface.  This function works just like a COM QueryInterface
 * call and is used if the object is not being aggregated.
 */
STDMETHODIMP_(NTSTATUS) CRTStream::NonDelegatingQueryInterface
(
    IN  REFIID  Interface,
    OUT PVOID * Object
)
{
    ...
    DOUT (DBG_PRINT, ("[CRTStream::NonDelegatingQueryInterface]"));

    //
    // Convert for IID_IRTMiniportStream
    //
    if (IsEqualGUIDAligned (Interface, IID_IRTMiniportStream))
    {
        *Object = (PVOID)(PRTMINIPORTSTREAM)this;
    }
    //
    ...
    {
        *Object = (PVOID)(PUNKNOWN)(PRTMINIPORTSTREAM)this;
    }
    else
    {
        *Object = NULL;
 ...
 */
STDMETHODIMP_(NTSTATUS) CRTStream::SetFormat
(
...
)
{
    ...
    DOUT (DBG_PRINT, ("[CRTStream::SetFormat]"));
...
)
{
    ...
    DOUT (DBG_PRINT, ("[CRTStream::SetContentId]"));
...
/*****************************************************************************
 * CRTStream::AllocateAudioBuffer
 *****************************************************************************
 * This functions allocates an audio buffer of the size specified and maps
 * it into the scatter gather table of the ICH DMA engine.
 ...
 */
STDMETHODIMP_(NTSTATUS) CRTStream::AllocateAudioBuffer
(
    IN  ULONG               size,
...
)
{
    ...
    //
    // Limit the size to 50ms of 48KHz, 16bit, 6ch audio data.
    //
    if (size > 28800)
      size = 28800;
    ...
    //
    // Allocate the buffer.
    // The ICH has problems playing 6ch data on page breaks (a page is 4096 bytes
    ...
    audioBufferMdl = PortStream->AllocateContiguousPagesForMdl (low, high, size);
    ...
    //
    PPFN_NUMBER pfns = MmGetMdlPfnArray (audioBufferMdl);
    ...
    *cacheType = MmCached;
    ...
    {
        BDList[loop].dwPtrToPhyAddress = (DWORD)(*pfns << PAGE_SHIFT);
...
)
{
    ...
    DOUT (DBG_PRINT, ("[CRTStream::PowerChangeNotify]"));
 ...
 */
STDMETHODIMP_(NTSTATUS) CRTStream::SetState
(
    IN  KSSTATE State
)
{
    ...
    DOUT (DBG_PRINT, ("[CRTStream::SetState]"));
    ...
    switch (State)
    {
        ...
        case KSSTATE_ACQUIRE:
            // Fail if we did not allocate the buffer yet.
            if (!audioBufferMdl)
                return STATUS_UNSUCCESSFUL;
            break;
 ...
 */
STDMETHODIMP_(NTSTATUS) CRTStream::GetPosition
(
...
)
{
    ...
    if ((!audioBufferMdl) || (DMAEngineState == DMA_ENGINE_OFF))
    {
    ...
    //
    if (RegisterX_PICB || nCurrentIndex || (DMAEngineState == DMA_ENGINE_ON))
    {
        //
        // The PlayOffset we assume is the position we got from the HW. The
        // WriteOffset is PlayOffset + FIFO size.
        //
        Position->PlayOffset = ((DWORD)(BDList[nCurrentIndex].wLength - RegisterX_PICB) << 1);
 ...
 */
NTSTATUS CRTStream::ResetDMA (void)
{
 ...
 */
NTSTATUS CRTStream::PauseDMA (void)
{
 ...
 * been preserved.
 */
NTSTATUS CRTStream::ResumeDMA (void)
{
