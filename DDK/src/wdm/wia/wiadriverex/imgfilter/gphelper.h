...
static Status GetEncoderGUIDFromImage(
    IN  Bitmap      *pOriginalBitmap,
    OUT CLSID       *pFormatEncoder)
{
    Status          status;
    CLSID           imageFormat;
...
static inline void CalculateBrightnessAndContrastParams( INT iBrightness, INT iContrast, float *scale, float *translate )
{
    //
    ...
    IStream         *pOutputStream)
{
    ...
    if (!pTargetBitmap || !pOutputStream)
    {
        hr = E_INVALIDARG;
    }
